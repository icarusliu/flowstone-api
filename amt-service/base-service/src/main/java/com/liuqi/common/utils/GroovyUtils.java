...
public class GroovyUtils {
     ...
     * @return 执行结果
     */
    public static Object execute(String groovy, Object params) {
     ...
     * @return 执行结果
     */
    public static Object execute(String groovy, Object params, Object params1) {
        ScriptEngine engine = scriptEngineManager.getEngineByName("groovy");
        Bindings binding = engine.createBindings();

        try {
            engine.eval(groovy, binding);
            return ((Invocable) engine).invokeFunction(FUNC_NAME, params, params1);
        } catch (Exception e) {
            log.error("执行groovy脚本异常, {}, {}", e.getMessage(), groovy);
            throw new RuntimeException("groovy脚本执行异常（" + e.getMessage() + ")", e);
        }
