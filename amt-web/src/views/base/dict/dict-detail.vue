...
const fields = [
    { label: '字典编码', prop: 'code', required: true, validation: {
        ...
        validator: (val, rule, callback) => {
            ...
            https.post('/base/dict/query', {code: model.value.code}).then(resp => {
                if (!resp?.length) {
                    return callback()
                }
...
function save() {
    formRef.value.validate(val => {
        ...
        if (model.value.id) {
            https.put('/base/dict/update', model.value).then(resp => {
                ElMessage.success('操作成功')
                emits('reload')
                editing.value = false
            })
        } else {
            https.post('/base/dict/add', model.value).then(resp => {
