<template>
    <div class="d-flex">
        ...
        <div class="flex-auto">
            <entity-manager apiPrefix="/sys/user" :fields="fields" :queryFields="queryFields" ref="entityManagerRef"
                :params="params">
                <template #rowButtons="{ row }">
                    <el-link type="danger" v-show="row.status && !row.isSuperAdmin" class="mr-2"
                        @click="updateStatus(row, 0)" v-perm="'update'">停用</el-link>
...
const fields = [
    ...
    { label: '部门', prop: 'deptId', type: 'tree-select', options: getDeptTree, converter: (val, row) => row.deptName },
    { label: '手机号', prop: 'phone', width: '200px' },
    {
        label: '状态', prop: 'status', width: '80px', tagType: (val) => {
            return {
