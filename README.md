...
# 重大更新
最新版本增加数据加工模块，提供数据抽取、数据加工与落库、依赖自动加载、血缘分析、MQTT动态监听、Kafka动态监听等ETL功能，可当成ETL工具或一个简化版的数据中台使用。结合现有的接口配置功能，可以将数据从各个系统抽取至统一的数据库，并基于抽取的数据进行分层数据加工与存储，再将处理的结果数据表配置成接口，提供给前端或数据展示工具使用。

# 1. 流石数据管理工具
采用前后端分离模式，前端使用vue3+element-plus等；后端使用Spring Boot+MyBatis Plus；主要包含以下两大类功能
## 接口配置功能
...
## 数据加工
通过数据加工功能，本工具可以当成一个简单的数据中台或ETL工具使用；
- 提供数据同步功能，可将不同库下的数据同步至统一库中；
- 提供实时数据监听功能，目前支持将MQTT或kafka消息进行整理加工后存储于数据库表中，同时支持当前记录与上一条记录的联合数据计算；
- 提供数据加工功能，使用SQL语句的方式完成数据的分层加工处理；
- 提供任务执行日志功能；
- 提供依赖根据脚本自动加载功能，可自动分析使用的表并添加相应的任务依赖；
- 提供自动血缘分析，可根据当前任务使用及更新的表，自动组装血缘树并进行展示；
## 基础功能
- 数据模型维护及数据预览等功能； 

# 2. 适用场景
- 当已经有了大屏或者其它前端低码工具，而后端接口仍旧需要定制开发时，可以使用本工具来进行后端接口的快速配置与开发；
- 在数据类的交付项目中，需要进行数据抽取、加工及开发对应的数据接口时，可以使用本工具来完成替代这部分功能，从而能够快速完成数据加工处理、接口开发，提升交付效率。
...
镜像地址：swr.cn-east-3.myhuaweicloud.com/icarus-tools/flowstone-amt:v1.1.2
...
```cmd
docker run -d -e SPRING_SECURITY_DOMAIN=localhost -p 3001:80 swr.cn-east-3.myhuaweicloud.com/icarus-tools/flowstone-amt:v1.1.2
```
...
- SPRING_SECURITY_DOMAIN可以修改域名，如不指定，默认是localhost，以下命令将域名修改成test.com
    ```cmd
    docker run -d -e SPRING_SECURITY_DOMAIN=test.com -p 3001:80 swr.cn-east-3.myhuaweicloud.com/icarus-tools/flowstone-amt:v1.1.2
    ```
...
## master主分支
- 基础组件优化
- ！增加数据加工大模块，包含跨库的数据同步、基于SQL语句的数据加工、定时调度、依赖配置等ETL功能；可以当成一个简易的ETL工具来使用，也可以当成一个简易的数据中台；
- 数据加工模块：增加MQTT监听功能，可以动态监听MQTT数据并进行处理与转换然后写入到MySql表中；
- 增加数据模型维护功能，可通过前端界面创建数据模型并生成对应的物理实体表，并支持内置的查询接口直接查询生成的表中的数据；
- 增加血缘分析功能，可自动分析加工任务所使用的表及更新的表，进行血缘分析，并以图形方式进行展示，方便进行数据来源追踪；
- 增加Oracle/PostgreSql接入支持；
- 界面框架优化，增加顶部菜单及引入相关个性化配置；
- 增加接口浏览功能及接口文档快速查看功能；
- 增加接口概览界面，展示接口相关统计数据及性能相关数据；
- 系统配置模块优化，增加角色用户机构权限等基础管理功能；
